package net.teamcrimx.party.api.party;

public class PartyConstants {

    public static final String PARTY_CHANNEL = "teamcrimx:party";

    public static final String PARTY_CREATION_MESSAGE = "party:creation";
    public static final String PARTY_INVITE_MESSAGE = "party:invite";
    public static final String PARTY_JOIN_MESSAGE = "party:join";
    public static final String PARTY_LEAVE_MESSAGE = "party:leave";
    public static final String PARTY_KICK_MESSAGE = "party:kick";
    public static final String PARTY_CLOSE_MESSAGE = "party:close";
    public static final String PARTY_PROMOTE_MESSAGE = "party:promote";
    public static final String PARTY_LIST_MESSAGE = "party:list";
    public static final String PARTY_CHAT_MESSAGE = "party:chat";

    public static final String HAS_PARTY_DOCUMENT_PROPERTY = "hasParty";
    public static final String PARTY_UUID_DOCUMENT_PROPERTY = "partyId";
    public static final String PARTY_INVITATIONS_DOCUMENT_PROPERTY = "partyInvites";

}
